...
from summary_gen import generate_study_material, _has_devanagari, _translate_to_english
from ai_chat import chat_with_ai
from mock_ai import generate_mock_test
from sample_paper_ai import generate_sample_paper
from study_pack_ai import generate_study_pack
...
def get_transcript(video_id):
    ...
    try:
        ...
        try:
            transcript = ytt.fetch(video_id, languages=['hi', 'en', 'en-US', 'en-GB'])
            return ' '.join([entry.text for entry in transcript])
        except Exception:
            return None
    ...
    if data_key:
        ...
        if not already_done:
            ...
            if changed:
                ...
                try:
                    from testdb import study_data_collection
...
def mock():
    ...
    return render_template("mock.html", has_summary=has_summary)
...
def sample_paper():
    ...
    data, _ = _cached_load_study_data(key)
    has_summary = data is not None
    return render_template("samplePaper.html", has_summary=has_summary)
